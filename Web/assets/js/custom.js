...
let menu = document.getElementById('navbar_toggle');
...
function changeMenuColor(color){
  ...
  let toggleLineElements = menu.querySelectorAll('.toggle-line');
  let toggleTextElement = menu.querySelector('.toggle-text');
  
  for (let i = 0; i < toggleLineElements.length; i++) {
    toggleLineElements[i].style.backgroundColor = color;
  }
  toggleTextElement.style.color = color;
}
...
if(window.innerWidth < 767){
  ...
  window.addEventListener('scroll', function() {
    ...
    if(window.scrollY > (window.innerHeight * 0.5) && window.scrollY < (window.innerHeight)){     
      changeMenuColor('white');
      menu.addEventListener('click', menuClickHandler);
    }else{
      changeMenuColor('black');
